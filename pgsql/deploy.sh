...
containerConfigPath="/etc/postgresql/conf"
containerDataPath="/var/lib/postgresql/data"
containerLogPath="/var/lib/pgsql_log"

Green "finished init container variables"

PgsqlRootUser="root"
PgsqlRootPw="YourMysqlRootPassword"
Yellow "please make sure that pg user root and password has modify to what you want"
Green "finished init container variables"

docker run -d \
-p 25432:5432 \
-e POSTGRES_USER=$PgsqlRootUser \
-e POSTGRES_PASSWORD=$PgsqlRootPw \
...
--restart always --name pgsql_1 postgres:12.18 \
-c "config_file=/etc/postgresql/conf/postgresql.conf"
